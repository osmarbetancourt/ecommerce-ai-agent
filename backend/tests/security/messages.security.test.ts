...
describe('Messages Security Middleware', () => {
  ...
  it('should block access to messages in another user conversation if not admin', async () => {
  ...
  it('should allow access to messages in own conversation', async () => {
    function ownershipMiddleware(req: any, res: any, next: any) {
  ...
  it('should allow admin to access any message', async () => {
    function ownershipMiddleware(req: any, res: any, next: any) {
      const conversation = { user_id: 456 };
      const userId = req.user?.id;
      const isAdmin = req.user?.role === 'admin';
